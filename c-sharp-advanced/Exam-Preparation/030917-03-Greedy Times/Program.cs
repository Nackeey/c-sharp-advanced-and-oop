...
class Program
{
    static void Main()
    {
        ...
        for (int i = 0; i < items.Length; i += 2)
        {
            ...
            if (itemType == "invalid" || !canInsertItem) continue;
    ...
    private static string PrintBag(Dictionary<string, long> bag, string type, long totalAmount)
    {
        ...
        result.AppendLine($"<{type}> ${totalAmount}");
    ...
    private static bool CanPutItemInBag(string itemType, long itemAmount, long bagCapacity,
        long goldQuantity, long gemQuantity, long cashQuantity)
    {
        long bagOccupied = goldQuantity + gemQuantity + cashQuantity;

        if (bagCapacity < bagOccupied + itemAmount) return false;
